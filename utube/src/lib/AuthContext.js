...
export const UserProvider = ({ children }) => {
  ...
  const handlegooglesignin = async () => {
    try {
      ...
      const { token, result: backendUser, locationState, theme } =
        response.data;
      ...
      // Proper login
      login(backendUser, locationState, theme);
    } catch (error) {
  ...
  const verifyOtpLogin = async (email, phone, otp) => {
    try {
      ...
      login(result, storedUser?.locationState, storedUser?.theme);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (firebaseuser) => {
      // Only logout if no firebase user AND no stored user
  ...
  /* =====================================================
     RESTORE USER ON PAGE LOAD (VERY IMPORTANT)
  ===================================================== */
  ...
  useEffect(() => {
    ...
    setLoading(false); // 🔥 stop hydration block
  }, []);
